...
Without this operator, cert-manager's trust-manager atomically swaps the old root for the new one at the moment of renewal. Any workload whose cert chains to the old root fails TLS verification immediately, before re-issuance can complete. This is what caused the incident on 2026-02-20 described in [01-problem-statement](../../architecture/01-problem-statement/README.md).
...
# Deploy via Helm template (values file supplies image tag, pkirotation.enabled, etc.)
