...
- Syncing trust material to anything outside this cluster's own trust-manager Bundles
...
`trustBundle` and `stagingConfigMap` are `RootCA`-only fields. They are ignored when `role: IntermediateCA`.
...
trust-manager detects intermediate Secret change → updates Bundle ConfigMap
```
...
| `reconcilePreservingOldRoot`, `reconcileDualTrustActive` | Unchanged — `RootCA` role only, never entered by `IntermediateCA` CRs |
