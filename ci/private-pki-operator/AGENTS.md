# private-pki-operator - AI Agent Guide

## This Project

**Module:** `github.com/adaptive-enforcement-lab/private-pki-operator/ci/private-pki-operator`
**Go version:** 1.26
**kubebuilder:** v4.13.0 / controller-runtime v0.23

### What it does
Automates root CA rotation in a cert-manager + trust-manager PKI hierarchy.
Implements a dual-trust window state machine so the old root stays trusted until
...
|---------|---------|---------|
| `github.com/cert-manager/cert-manager` | v1.19.4 | API types for Certificate, ClusterIssuer, CertificateRequest |
| `github.com/cert-manager/trust-manager` | v0.21.1 | API types for Bundle |
| `sigs.k8s.io/gateway-api` | (transitive) | Required by cert-manager v1.19 |

### CRD
`PKIRotation` — cluster-scoped, group `platform.adaptive-enforcement-lab.com/v1alpha1`, short name `pkir`.

Spec fields:
- `rootCA.certificateName` / `rootCA.certificateNamespace` — cert-manager Certificate for the root CA
- `trustBundle.name` — cluster-scoped trust-manager Bundle name
- `trustBundle.stagingConfigMap.name` / `.namespace` — ConfigMap the operator writes PEMs to (must exist, never owned by cert-manager)
- `reissuanceTimeout` — default `15m`

### internal/pki package
Pure stdlib package (no Kubernetes deps). All X.509 and PEM logic lives here.

| Function | File | Description |
|----------|------|-------------|
| `SKIDFromPEM([]byte) (string, error)` | `x509.go` | Extract SubjectKeyId as `"AA:BB:CC"` hex. Errors if absent. |
| `AKIDFromPEM([]byte) (string, error)` | `x509.go` | Extract AuthorityKeyId. Returns `""` (no error) if absent. |
| `AppendPEM(existing, newCert []byte) []byte` | `pem.go` | Idempotent DER-based PEM concatenation. |
| `RemovePEMBySKID(pemData []byte, skid string) ([]byte, error)` | `pem.go` | Remove first cert matching SKID from PEM bundle. |

### Helm chart
`charts/private-pki-operator/` (relative to repo root `private-pki-operator/`).

```
charts/private-pki-operator/
├── crds/pkirotations.yaml          ← copy of generated CRD (update after make manifests)
├── values.yaml                     ← pkirotation.enabled=false by default
└── templates/
    ├── shared/serviceaccount.yaml
    └── features/k8s/
        ├── clusterrole.yaml
        ├── clusterrolebinding.yaml
        ├── leader-election-role.yaml   ← Role + RoleBinding combined
        ├── deployment.yaml
        └── pkirotation.yaml            ← staging ConfigMap + PKIRotation CR (gated)
```

### CI pipeline
`.github/workflows/build.yml` — triggers on any push/PR. Two jobs:
`go build/vet/test` (cmd, api, internal/pki — the envtest-based
`internal/controller` suite is run locally via `make test`, not in CI) and
`helm lint` for both charts. `.github/workflows/trufflehog.yaml` runs secret
scanning on every push via the public TruffleHog action. Neither dispatches
into any external reusable workflow — both are self-contained.

Image tags: `make build-push TAG=vX.Y.Z` pushes
`ghcr.io/adaptive-enforcement-lab/private-pki-operator:vX.Y.Z`. There is no
automated release/publish step — bump the tag and push manually.

### Deployment workflow

The consuming cluster's own repo owns its override values (e.g. homelab's
`k8s/private-pki-operator/values.yaml`), not this repo. From that repo:

```bash
helm upgrade --install private-pki-operator <path-to-this-repo>/charts/private-pki-operator \
  -n cert-manager -f k8s/private-pki-operator/values.yaml
```

To enable the `PKIRotation` CR specifically: set `pkirotation.enabled: true`
in that values file (defaults to `false` so installing the chart never
takes ownership of a cluster's trust root by accident).

### When you regenerate manifests
After editing `*_types.go` or RBAC markers:
```bash
make manifests generate
```
`make manifests` automatically syncs `config/crd/bases/*.yaml` → `charts/private-pki-operator/crds/`.
